...
@RestController
@Tag(
    name = "5. User Profile API",
    description =
        "APIs for managing profile management and accessed with authentication token only.")
...
public class CustomerController {
  ...
  public ResponseEntity<UserListingResponse> getUsers(
      ...
      @RequestParam(name = "sortOrder", defaultValue = AppConstants.SORT_DIR, required = false)
          String sortOrder) {
    UserListingResponse userListResponse =
  ...
  @Operation(
      ...
      description =
          "Updates the delivery address for a specific user. Only valid keys (address1, address2, city, state, zipCode, country) are accepted.")
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            ...
            content =
                @Content(
                    schema =
                        @Schema(example = "{\"error\": \"Invalid keys found: [invalidKey]\"}"))),
