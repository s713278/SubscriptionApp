...
public class VendorController {
  ...
  public ResponseEntity<APIResponse<?>> getVendorProfileById(
    ...
    var response =
        serviceManager.getVendorService().fetchFullVendorDetailsById(vendorId, userAuthentication);
    return new ResponseEntity<>(
        APIResponse.success(HttpStatus.OK.value(), response), HttpStatus.OK);
  }

  @Operation(
      summary = "Search vendor profile by mobile",
  ...
  public ResponseEntity<APIResponse<?>> getVendorProfileByMobile(
      @PathVariable String mobile, Authentication authentication) {
    log.debug("Request received for fetching vendor profile for mobile #{}", mobile);
    UserAuthentication userAuthentication = (UserAuthentication) authentication;
    var response =
        serviceManager.getVendorService().fetchVendorByMobile(mobile, userAuthentication);
    return new ResponseEntity<>(
        APIResponse.success(HttpStatus.OK.value(), response), HttpStatus.OK);
  }
