...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<APIErrorResponse> handleAPIErrorResponse(APIException apiError) {
    ...
    APIErrorResponse apiErrorResponse =
        new APIErrorResponse(
            apiError.getApiErrorCode(), apiError.getFailureReason(), apiError.getDetails());
    return new ResponseEntity<>(apiErrorResponse, HttpStatus.valueOf(apiError.getStatus()));
  }

  @ExceptionHandler(ConstraintViolationException.class)
  public ResponseEntity<APIErrorResponse> myConstraintsViolationException(
  ...
  public ResponseEntity<APIErrorResponse> handleRuntimeException(
    ...
    APIErrorResponse apiError =
        new APIErrorResponse(
            APIErrorCode.API_500, e.getMessage(), List.of(request.getDescription(false)));
  ...
  public ResponseEntity<APIErrorResponse> handleAllExceptions(Exception ex, WebRequest request) {
    ...
    APIErrorResponse apiError =
        new APIErrorResponse(
            APIErrorCode.API_500, ex.getMessage(), List.of(request.getDescription(false)));
    return new ResponseEntity<>(apiError, HttpStatus.INTERNAL_SERVER_ERROR);
  }
}
