...
public abstract class AbstractVendorService {
    ...
    protected VendorProfileResponse validateOwnershipAndGet(Long vendorId, Long userPrincipal){
        ...
        if(!Objects.equals(vendor.getUserId(), userPrincipal)){
            throw new APIException(APIErrorCode.API_403,"Unauthorized access to fetch vendor profile "+vendorId);
        }
    ...
    protected VendorProfileResponse fetchVendorById(Long vendorId) {
        var vendor =  repoManager.getVendorRepo().findById(vendorId)
                .orElseThrow(()-> new APIException(APIErrorCode.API_404,"Vendor profile not existed for vendor #"+vendorId));
        return modelMapper.map(vendor, VendorProfileResponse.class);
    }

    protected Set<String> getRoles(UserAuthentication userAuthentication){
        if(userAuthentication.getAuthorities().isEmpty()){
            throw new APIException(APIErrorCode.API_403,"User #"+userAuthentication.getPrincipal()+"have no access to fetch vendor profile ");
        }
        return userAuthentication.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toSet());
    }

    @Transactional
    public final CreateItemResponse createVendorProfile(VendorProfileRequest vendorRequest, Authentication authentication) {
        ...
        if (isAdmin || isCustomerCare){ //User and Vendor Profile has to be created
           ...
           // return validateOwnershipAndGet(vendorId,userId);
            ...
            throw new APIException(APIErrorCode.API_401,String.format("User %s not allowed to have vendor profile",principal));
        }
    ...
    protected void postCreateVendorProfile(Long vendorId,String mobileNumber){
        ...
        notificationContext.notifyUser(NotificationType.SMS, NotificationTemplate.PROFILE_UPDATES, mobileNumber);
    }


    @Transactional
    public final CreateItemResponse updateVendorProfile(VendorProfileRequest vendorRequest, Long vendorId,Authentication authentication) {
        ...
        return new CreateItemResponse(vendorEntity.getId(),String.format("Vendor profile:%s updated by %s successfully.",vendorId,principal));
    }
    ...
    protected Vendor preUpdateVendorProfile(VendorProfileRequest vendorRequest,Long vendorId){
        ...
        if(VendorStatus.SUSPENDED == vendor.get().getStatus()){
            throw new APIException(APIErrorCode.API_400,"Vendor profile is suspended state,No further updates allowed.");
        }
        return vendor.get();
    }
    ...
    public PaginationResponse<VendorProfileRequest> fetchAllVendors(Integer pageNumber, Integer pageSize, String sortBy, String sortOrder) {
        ...
        List<VendorProfileRequest> storeDTOs = stores.stream().map(store -> modelMapper.map(store, VendorProfileRequest.class))
    ...
    public VendorProfileResponse fetchVendorByMobile(String mobileId, Authentication authentication) {
        log.debug("Fetching vendor profile for mobile #{} buy user #{}",mobileId,authentication.getPrincipal());
        UserAuthentication userAuthentication= (UserAuthentication)authentication;
        Long userId=(Long)userAuthentication.getPrincipal();
        var roles=getRoles(userAuthentication);
        boolean isAdmin = roles.contains(UserRoleEnum.ADMIN.name());
        ...
        if(optionalVendor.isEmpty()){
            throw new APIException(APIErrorCode.API_404,"Vendor not existed with this number #"+mobileId);
        }
        ...
        log.warn("User #{} doesn't have access to view vendor profile #{}",userId,optionalVendor.get().getId());
        throw new APIException(APIErrorCode.API_403,"Unauthorized to fetch vendor profile");
    }
}
