...
public class EmailSignUpService extends AbstractSignUpService<EmailSignUpRequest> {
  ...
  protected void postSignUpOperations(SignUpDTO request) {
    ...
    // EmailActivationEvent activationEvent = new EmailActivationEvent(this, request.getEmail(),
    // request.getEmailActivationtoken(),request.getOtp());
  ...
  protected SignUpDTO doSignUp(EmailSignUpRequest request) {
    ...
    customer.setPassword(
        passwordEncoder.encode(request.getPassword())); // Use BCrypt for password encryption

    // Fetch the role and ensure it is managed
    Role role =
        repoManager
            .getRoleRepo()
            .findById(AppConstants.USER_ROLE_ID)
    ...
    customer.setOtpExpiration(
        LocalDateTime.now().plusMinutes(5)); // Set OTP expiration to 5 minutes
    ...
    customer.setEmailTokenExpiration(
        LocalDateTime.now()
            .plusSeconds(
                globalConfig.getCustomerConfig().getEmailTokenExp())); // Set token expiration time
