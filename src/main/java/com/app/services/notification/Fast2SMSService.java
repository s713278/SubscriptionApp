...
public class Fast2SMSService extends SMSService {
  ...
  public void sendTextMessage(String mobileNo, String otp) {
    log.info(
        "Is OTP sending and verification is enabled ? {} ",
        globalConfig.getCustomerConfig().isOtpVerificationEnabled());
    ...
    try {
      ...
      if (response.getStatusCode() == HttpStatus.OK) {
        ...
        //  throw new RestClientException("Failed to send SMS, status: " +
        // response.getStatusCode());
      }
    } catch (Exception e) {
      log.error("OTP failed to send for mobile number : {}", mobileNo);
      //  throw new RestClientException("Exception occurred while sending SMS: " + e.getMessage(),
      // e);
    }
  }
}
