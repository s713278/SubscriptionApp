...
public abstract class AbstractCreateSubscriptionService {
  ...
  protected void preSubscription(Long userId, CreateSubscriptionRequest request) {
    if (serviceManager.getSubscriptionService().fetchByUserIdAndSkuId(userId, request.getSkuId())) {
      throw new APIException(
          APIErrorCode.API_409, "Subscription already existed for sku id : " + request.getSkuId());
    }
    var skuSub =
        serviceManager
    ...
    if (request.getFrequency() == SubFrequency.ONE_TIME) {
      ...
      if (deliveryDaysList != null
          ...
          && "FIXED".equalsIgnoreCase((String) map.get(AppConstants.SUB_ONE_TIME_SUB_TYPE))) {
