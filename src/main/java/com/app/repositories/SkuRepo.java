...
public interface SkuRepo extends JpaRepository<Sku, Long> {
  ...
  @Query(
      value =
        ...
        SELECT
            ...
            JSON_AGG(
                JSON_BUILD_OBJECT(
                   'id', tsp.id,
                   'frequency', tsp.frequency,
                   'eligible_delivery_days',COALESCE(tssp.eligible_delivery_days->'delivery_days', '[]')
                )
        ...
        """,
      nativeQuery = true)
  List<SkuProjection> findSkusVendorProductId(@Param("vendorProductId") Long vendorProductId);
}
