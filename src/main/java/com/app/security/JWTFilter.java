...
public class JWTFilter extends OncePerRequestFilter {
    ...
    if (authHeader != null && !authHeader.isBlank() && authHeader.startsWith("Bearer ")) {
      ...
      if (jwt.isBlank()) {
        ...
        APIErrorResponse apiError =
            new APIErrorResponse(APIErrorCode.API_401, "Invalid JWT token in Bearer Header");
        ...
        try {
          ...
          apiAuthValidator
              .authenticateUser(Long.parseLong(userId))
  ...
  private void extracted(HttpServletResponse response, Exception exception) throws IOException {
    ...
    if (exception instanceof APIException apiException) {
      apiError =
          new APIErrorResponse(apiException.getApiErrorCode(), apiException.getFailureReason());
    } else {
